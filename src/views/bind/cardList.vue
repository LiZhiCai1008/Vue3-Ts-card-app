  ...
  <Page isHeader>
    <div class="content lk-p-b-60">
      ...
      <div v-show="status == 0" class="no-card">
        ...
        >绑定储值卡</van-button>
        <div class="bind-other-card lk-m-b-20 lk-m-t-20 content">切换手机号</div>
      </div>
      <!-- 当前卡绑定其他手机号 -->
    ...
    >
      <template v-slot:content>
        <p class="agreement-content">
          ...
          3、储值卡遗失后，可自行挂失，挂失成功后，可立即办理补卡手续，同时需交补卡费5元/张，原卡号内的金额将转入新卡，同时旧卡作废。<br>
...
import { defineComponent, computed, reactive, toRefs, getCurrentInstance } from "vue";
...
export default defineComponent({
  ...
  setup() {
    ...
    const state = reactive<{ status: number }>({
      status: 0
    })
